...
def extract_show_information(show):
    ...
    show_date = (show_info.date).strftime("%Y-%m-%d")
...
all_promotions_total_attendance.to_csv('CSV files/Historical CSV files' + today + ' - All Japan attendance stats.csv', encoding='utf-8')
print('Current show stats done.')
#%% remove duplicates


all_promotions_df = pd.read_csv('CSV files/Historical CSV files' + today + ' - All Japan attendance stats.csv')
all_promotions_df.drop_duplicates(inplace=True)
all_promotions_df.to_csv('CSV files/Historical CSV files' + today + ' - All Japan attendance stats.csv', index=False)
